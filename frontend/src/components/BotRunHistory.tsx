    ...
    <div className="bg-surface border border-border rounded-lg p-3">
      ...
      {hasMore && (
        <button
          ...
          className="mt-1.5 text-[11px] text-muted hover:text-text underline"
        >
          {expanded ? '접기' : '전체 보기'}
        </button>
      )}
...
export function BotRunHistory({ agentId }: { agentId: string }) {
  ...
  return (
    <div className="space-y-3">
      {/* Stats row */}
      {stats && (
        <div className="flex items-center gap-4 text-[12px] text-muted bg-surface border border-border rounded-lg px-4 py-2.5">
          <span>총 실행 <span className="text-text font-medium">{stats.total_runs}회</span></span>
          ...
          <button
            onClick={() => refetch()}
            disabled={isFetching}
            className="ml-auto flex items-center gap-1 text-muted hover:text-text disabled:opacity-50"
          >
      ...
      {itemsLoading ? (
        ...
        <div className="text-center py-8 text-muted text-[13px] bg-surface border border-border rounded-lg">
          <p>아직 실행 기록이 없습니다</p>
          {!stats && (
            <button
              onClick={() => refetch()}
              disabled={isFetching}
              className="mt-3 flex items-center gap-1.5 px-3 py-1.5 text-[12px] text-muted hover:text-text border border-border rounded mx-auto"
            >
              <RefreshCw size={12} className={isFetching ? 'animate-spin' : ''} />
              새로고침
            </button>
