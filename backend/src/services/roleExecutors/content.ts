...
type OutputType = 'informational' | 'business' | 'narrative' | 'blog' | 'linkedin' | 'default'
...
function parseContentParams(task: string): { outputType: OutputType; platform: Platform; cleanTask: string } {
  ...
  // outputType 파싱
  const infoMatch = task.match(/\[outputType:(informational|business|narrative|blog|linkedin)\]/i)
  if (infoMatch) {
