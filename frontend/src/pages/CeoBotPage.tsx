...
import { api, feedApi, type Agent, type FeedItem } from '../lib/api'
import { useAppStore } from '../store/app.store'
import { BotRunModal } from '../components/BotRunModal'
import { formatDistanceToNow } from 'date-fns'
import { ko } from 'date-fns/locale'
import { Play, Loader2, AlertCircle, Clock, CheckCircle2, XCircle } from 'lucide-react'
...
export default function CeoBotPage() {
  ...
  const { data: summaryData, isLoading: summaryLoading } = useQuery<CeoSummaryData>({
    queryKey: ['ceo-summary', missionId],
    queryFn: () =>
      api
        .get<{ data: CeoSummaryData }>('/api/ceo/summary', {
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-muted">
            {currentMission?.name} — Solo Factory OS 전체 현황
          </p>
        </div>
        {summaryLoading && (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <KpiCard
            ...
            label="승인 대기"
            value={feedSummary.approvals_pending}
            suffix="건"
          ...
          <KpiCard
            ...
            label="오류"
            value={feedSummary.errors_today}
            suffix="건"
            color="text-red-400"
          />
          <KpiCard
            ...
            label="완료"
            value={feedSummary.completed_today}
            suffix="건"
      ...
      <div>
        ...
        <div className="space-y-2">
          {feedLoading ? (
            ...
            <div className="bg-surface border border-border rounded-lg p-6 text-center text-muted text-[13px]">
              아직 피드 항목이 없습니다. 봇을 실행해보세요.
            </div>
          ) : (
            feedItems.map(item => (
...
function BotCard({
  ...
  if (lastRunStatus === 'result') {
    statusIcon = <CheckCircle2 size={12} className="text-green-400" />
    ...
    <div className="bg-surface border border-border rounded-lg p-4 hover:border-primary/50 transition-colors">
      ...
      <div className="space-y-1.5 mb-4">
        <div className="flex items-center gap-1.5 text-[11px] text-muted">
          {statusIcon}
          <Clock size={11} />
          <span>마지막 실행: {formatRelativeTime(agent.last_run_at)}</span>
        </div>
        <div className="text-[11px] text-muted">
          총 {(agent.run_count ?? 0).toLocaleString()}회 실행
        </div>
      </div>
