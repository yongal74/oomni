...
export default function PinPage() {
  ...
  useEffect(() => {
    fetch('/api/auth/google/status')
      .then(r => r.json())
      .then((data: { google_configured: boolean }) => {
  ...
  const handleGoogleLogin = () => {
    // 시스템 브라우저에서 Google OAuth 시작
    if (window.electronAPI?.openExternal) {
      window.electronAPI.openExternal('http://localhost:3001/api/auth/google')
    } else {
    ...
    <div className="min-h-screen bg-bg flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-surface border border-border rounded-xl p-6">
          ...
          {/* Google 로그인 버튼 (credentials 있을 때만 표시) */}
          {googleConfigured && !isSetup && (
            <>
              ...
              <button
                ...
                disabled={googlePolling}
                className="w-full py-2.5 bg-surface border border-border rounded-lg text-[13px] font-medium text-text hover:border-[#444] transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {googlePolling ? (
                  <>
