...
// ── 태스크 분류 ───────────────────────────────────────────────────────────────
type OpsTrack = 'n8n' | 'report' | 'finance' | 'monitor' | 'security' | 'general'
...
function classifyTask(task: string): OpsTrack {
  ...
  if (t.includes('보안') || t.includes('security') || t.includes('owasp') || t.includes('취약') || t.includes('audit') || t.includes('rls') || t.includes('npm audit') || t.includes('감사')) return 'security'
  return 'general'
}
...
const SYSTEM_SECURITY = `당신은 애플리케이션 보안 전문가입니다.
OWASP Top 10, 의존성 취약점, 인프라 보안을 체계적으로 분석합니다.
...
export async function opsExecutor(ctx: ExecutorContext): Promise<void> {
  ...
  const systemPrompt = track === 'n8n' ? SYSTEM_N8N
    ...
    : track === 'security' ? SYSTEM_SECURITY
    : SYSTEM_GENERAL
...
${JSON.stringify((runStats.rows as any[]).map((r: any) => r.role).filter((v: string, i: number, a: string[]) => a.indexOf(v) === i), null, 2)}`
    ...
    send('stage', { stage: 'scanning', label: '보안 감사 실행 중...' })
    userMessage = `보안 감사 요청: ${task}

현재 시스템 현황:
- 실행 중인 봇: ${JSON.stringify((runStats.rows as any[]).map((r: any) => r.role).filter((v: string, i: number, a: string[]) => a.indexOf(v) === i))}
- 최근 에러: ${JSON.stringify((errorItems.rows as any[]).slice(0, 5))}

위 정보를 기반으로 OWASP Top 10 기준 보안 감사 리포트를 작성하고 즉시 적용 가능한 보안 강화 방안을 제시해주세요.`
  } else {
...
위 데이터를 바탕으로 ${trackLabel}을 작성해주세요.`
  ...
  send('ops_done', { track, isN8nWorkflow: track === 'n8n', isSecurityAudit: track === 'security' })
}
