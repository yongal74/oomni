...
const ROLE_INSTRUCTIONS: Record<AgentRole, string> = {
  ...
  design: 'Pencil MCP를 활용해 디자인 파일을 생성하고 OOMNI API로 파일 경로를 보고하라.',
  ...
  n8n: 'n8n API를 통해 워크플로우를 생성/관리하고, 배포 결과를 OOMNI API로 보고하라.',
...
`;
  ...
  async run(agent: Agent, task?: string, sessionId?: string): Promise<RunResult> {
    ...
    const args = sessionId
      ? ['--resume', sessionId, '--print', '-', '--output-format', 'stream-json']
